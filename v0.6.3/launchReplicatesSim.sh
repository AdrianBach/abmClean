...
prd_ctch_pry1_1=0.04  # argv[29] predator 1 prey1 catch probability
prd_ctch_pry2_1=0.04  # argv[30] predator 1 prey2 catch probability
...
#        prd_cull_quot_1=${prd1qotArray[$i]}
        ...
        #     prd_offs_1=${predOffsArray[$m]}  
            ...
            #     divide=$((3*$prd_surv_1)); by=$freq_surv; prd_cons_1=`echo "scale=0; ($divide+$by-1)/$by" | bc`;
                ...
                p=$(($p+1))
                printf "pry_cull_quot_1=$pry_cull_quot_1 \t # argv[$p] prey 2 culling quota\n" >> paramFile.txt
                p=$(($p+1))
                printf "pry_cull_impl_1=$pry_cull_impl_1 \t # argv[$p] prey 2 start of culling policy\n" >> paramFile.txt
                p=$(($p+1))
                printf "pry_freq_cull_1=$pry_freq_cull_1 \t # argv[$p] prey 2 culling intervention frequency\n" >> paramFile.txt
                p=$(($p+1))
                printf "prd_cull_quot_1=$prd_cull_quot_1 \t # argv[$p] predator 1 culling quota\n" >> paramFile.txt
                p=$(($p+1))
                printf "prd_cull_impl_1=$prd_cull_impl_1 \t # argv[$p] predator 1 start of culling policy\n" >> paramFile.txt
                p=$(($p+1))
                printf "prd_freq_cull_1=$prd_freq_cull_1 \t # argv[$p] predator 1 culling intervention frequency\n\n" >> paramFile.txt
                p=$(($p+1))
